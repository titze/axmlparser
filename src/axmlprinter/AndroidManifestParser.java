...
public class AndroidManifestParser {
	...
	private static class ManifestHandler extends DefaultHandler {
		...
		private SRPNode mCurrentNode = null;
				...
				if (mValidLevel == mCurrentLevel) {
					...
					case LEVEL_TOP:
						if (AndroidManifest.NODE_MANIFEST.equals(localName)) {
							...
							mManifestData.mPackage = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_PACKAGE, false /* hasNamespace */);
					...
					case LEVEL_INSIDE_MANIFEST:
						if (AndroidManifest.NODE_APPLICATION.equals(localName)) {
							value = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_PROCESS, true /* hasNamespace */);
							if (value != null) {
								mManifestData.addProcessName(value);
							}

							value = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_DEBUGGABLE, true /* hasNamespace */);
							if (value != null) {
							...
							mManifestData
									.setMinSdkVersionString(getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_MIN_SDK_VERSION, true /* hasNamespace */));
							mManifestData
									.setTargetSdkVersionString(getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_TARGET_SDK_VERSION, true /* hasNamespace */));
						} else if (AndroidManifest.NODE_USES_PERMISSION.equals(localName)) {
							mManifestData.addUsesPermission(attributes.getValue(0));
						} else if (AndroidManifest.NODE_PERMISSION.equals(localName)) {
							mManifestData.addPermissions(attributes.getValue(0));
							...
							// get the name
							value = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
							if (value != null) {
								feature.mName = value;
							}

							// read the required attribute
							value = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_REQUIRED, true /* hasNamespace */);
							if (value != null) {
							...
							value = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_GLESVERSION, true /* hasNamespace */);
							if (value != null) {
								try {
					...
					case LEVEL_INSIDE_APPLICATION:
						if (AndroidManifest.NODE_ACTIVITY.equals(localName)) {
							...
							mValidLevel++;
						} else if (AndroidManifest.NODE_ACTIVITY_ALIAS.equals(localName)) {
							processActivityNode(attributes);
							mValidLevel++;
					...
					case LEVEL_INSIDE_APP_COMPONENT:
						...
						if (mCurrentActivity != null && AndroidManifest.NODE_INTENT.equals(localName)) {
							...
							mCurrentActivity.addIntentFilter();
							// If exported status has not been set explicitly,
							// apply default
							if (!mCurrentActivity.isExported() && !mCurrentActivity.isNotExported()) {
								mCurrentActivity.setExported(true);
							}
							mValidLevel++;
							// Set intent-filter priority
							if (name.equals("intent-filter")) {
								if (attributes.getValue("android:priority") != null) {
									mCurrentActivity.getLastIntentFilter().setPriority(Integer.parseInt(attributes.getValue("android:priority")));
								}
							}
						}
						// SRP
						if (mCurrentNode != null && AndroidManifest.NODE_INTENT.equals(localName)) {
							mCurrentNode.addIntentFilter();
							mValidLevel++;
							// Set intent-filter priority
							if (name.equals("intent-filter")) {
								if (attributes.getValue("android:priority") != null) {
									mCurrentNode.getLastIntentFilter().setPriority(Integer.parseInt(attributes.getValue("android:priority")));
								}
							}
						}
						break;
					case LEVEL_INSIDE_INTENT_FILTER:
						if (mCurrentActivity != null) {
							if (AndroidManifest.NODE_ACTION.equals(localName)) {
								...
								if (action != null) {
									...
									mCurrentActivity.getLastIntentFilter().addAction(action);
								}
							} else if (AndroidManifest.NODE_CATEGORY.equals(localName)) {
								String category = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
								if (CATEGORY_LAUNCHER.equals(category)) {
									mCurrentActivity.setHasLauncherCategory(true);
								}
								mCurrentActivity.getLastIntentFilter().addCategory(category);
							} else if (AndroidManifest.NODE_DATA.equals(localName)) {
								StringBuilder attrString = new StringBuilder();
								for (int i = 0; i < attributes.getLength(); i++) {
									attrString.append(attributes.getQName(i) + "=\"" + attributes.getValue(i) + "\", ");
								}
								if (attrString.length() > 0) {
									mCurrentActivity.getLastIntentFilter().addData(attrString.substring(0, attrString.length() - 2));
								}
							}
						...
						if (mCurrentNode != null) {
							...
							if (AndroidManifest.NODE_ACTION.equals(localName)) {
								...
								String action = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
								if (action != null) {
									mCurrentNode.getLastIntentFilter().addAction(action);
								}
							} else if (AndroidManifest.NODE_CATEGORY.equals(localName)) {
								String category = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
								mCurrentNode.getLastIntentFilter().addCategory(category);
							} else if (AndroidManifest.NODE_DATA.equals(localName)) {
								StringBuilder attrString = new StringBuilder();
								for (int i = 0; i < attributes.getLength(); i++) {
									attrString.append(attributes.getQName(i) + "=\"" + attributes.getValue(i) + "\", ");
								...
								if (attrString.length() > 0) {
									mCurrentNode.getLastIntentFilter().addData(attrString.substring(0, attrString.length() - 2));
								}
							}
							// if
							// (AndroidManifest.NODE_ACTION.equals(localName)) {
							// // get the name attribute
							// String action = getAttributeValue(attributes,
							// AndroidManifest.ATTRIBUTE_NAME,
							// true /* hasNamespace */);
							// if (action != null) {
							// mCurrentNode.addAction(action);
							// }
							// } else if
							// (AndroidManifest.NODE_CATEGORY.equals(localName))
							// {
							// String category = getAttributeValue(attributes,
							// AndroidManifest.ATTRIBUTE_NAME,
							// true /* hasNamespace */);
							// if (category != null) {
							// mCurrentNode.setCategory(category);
							// }
							// }
						}
						break;
			...
			try {
				...
				if (mValidLevel == mCurrentLevel) {
					...
					case LEVEL_INSIDE_APP_COMPONENT:
						...
						// launcher activity!
						if (mManifestData.mLauncherActivity == null && mCurrentActivity != null && mCurrentActivity.isHomeActivity()
		...
		private void processActivityNode(Attributes attributes) {
			...
			String activityName = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
			...
			String processName = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_PROCESS, true /* hasNamespace */);
			if (processName != null) {
		 ...
		 *            the attributes for the activity node.
		 * @param superClassName
		 *            the fully qualified name of the super class that this node
		 *            is representing
		 */
		private void processNode(Attributes attributes, String superClassName) {
			...
			if (serviceName != null) {
				...
				if (mErrorHandler != null) {
					mErrorHandler.checkClass(mLocator, serviceName, superClassName, false /* testVisibility */);
				}
				// get the exported flag.
				String exportedStr = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_EXPORTED, true);
				...
				if (l == null) {
					l = new ArrayList<SRPNode>();
					srpToNodeHash.put(superClassName, l);
				}
				l.add(mCurrentNode);
		 ...
		 * 
		 * @param attributes
		 *            the attributes for the instrumentation node.
		 */
		private void processInstrumentationNode(Attributes attributes) {
			// lets get the class name, and check it if required.
			String instrumentationName = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_NAME, true /* hasNamespace */);
			if (instrumentationName != null) {
				String instrClassName = AndroidManifest.combinePackageAndClassName(mManifestData.mPackage, instrumentationName);
				String targetPackage = getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_TARGET_PACKAGE, true /* hasNamespace */);
				mManifestData.mInstrumentations.add(new Instrumentation(instrClassName, targetPackage));
				if (mErrorHandler != null) {
					mErrorHandler.checkClass(mLocator, instrClassName, SdkConstants.CLASS_INSTRUMENTATION, true /* testVisibility */);
				}
			}
		}
		 ...
		 * 
		 * @param attributes
		 *            the attributes for the supports-screens node.
		 */
		private void processSupportsScreensNode(Attributes attributes) {
			...
			mManifestData.mSupportsScreensFromManifest
					.setResizeable(getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_RESIZEABLE, true /* hasNamespace */));

			mManifestData.mSupportsScreensFromManifest
					.setAnyDensity(getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_ANYDENSITY, true /* hasNamespace */));

			mManifestData.mSupportsScreensFromManifest
					.setSmallScreens(getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_SMALLSCREENS, true /* hasNamespace */));

			mManifestData.mSupportsScreensFromManifest
					.setNormalScreens(getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_NORMALSCREENS, true /* hasNamespace */));

			mManifestData.mSupportsScreensFromManifest
					.setLargeScreens(getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_LARGESCREENS, true /* hasNamespace */));
		}
		...
		private void processUsesConfiguration(Attributes attributes) {
			...
			mManifestData.mUsesConfiguration.mReqFiveWayNav = getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_REQ_5WAYNAV, true /* hasNamespace */);
			mManifestData.mUsesConfiguration.mReqNavigation = Navigation
					.getEnum(getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_REQ_NAVIGATION, true /* hasNamespace */));
			mManifestData.mUsesConfiguration.mReqHardKeyboard = getAttributeBooleanValue(attributes, AndroidManifest.ATTRIBUTE_REQ_HARDKEYBOARD, true /* hasNamespace */);
			mManifestData.mUsesConfiguration.mReqKeyboardType = Keyboard
					.getEnum(getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_REQ_KEYBOARDTYPE, true /* hasNamespace */));
			mManifestData.mUsesConfiguration.mReqTouchScreen = TouchScreen.getEnum(getAttributeValue(attributes, AndroidManifest.ATTRIBUTE_REQ_TOUCHSCREEN,
					true /* hasNamespace */));
		}
		...
		private String getAttributeValue(Attributes attributes, String attributeName, boolean hasNamespace) {
			...
			for (int i = 0; i < count; i++) {
				if (attributeName.equals(attributes.getLocalName(i))
						&& ((hasNamespace && SdkConstants.NS_RESOURCES.equals(attributes.getURI(i))) || (hasNamespace == false && attributes.getURI(i).length() == 0))) {
		...
		/**
		 * Searches through the attributes list for a particular one and returns
		 * its value as a Boolean. If the attribute is not present, this will
		 * return null.
		 * 
		 * @param attributes
		 *            the attribute list to search through
	 ...
	 */
	public static ManifestData parse(IAbstractFile manifestFile, boolean gatherData, ManifestErrorHandler errorHandler) throws SAXException, IOException,
	...
	public static ManifestData parse(InputStream manifestFileStream) throws SAXException, IOException, StreamException, ParserConfigurationException {
		if (manifestFileStream != null) {
			// BOMInputStream bomis = new BOMInputStream(manifestFileStream);
			InputSource is = new InputSource(manifestFileStream);
			...
			parser.parse(is, manifestHandler);
